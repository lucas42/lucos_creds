...
`lucos_creds` is used day-to-day for `development` and `production` credentials. Test environments were believed to be a greenfield pattern when this ADR was requested — but they are not. An audit ([lucas42/lucos_creds#361](https://github.com/lucas42/lucos_creds/issues/361)) surfaced at least one **already in active production use**: `lucos_contacts/test`. The lucos_contacts CI test job pulls it (`.circleci/config.yml`: `scp -s -P 2202 -o BatchMode=yes tests@creds.l42.eu:$CIRCLE_PROJECT_REPONAME/test/.env .`, with no error suppression, then `rm -f .env` after the run), and CircleCI build 4563 shows that `scp` exiting 0 — so the environment exists and is non-empty. So this ADR is **formalising and documenting an existing-but-undocumented practice**, not inventing one; that strengthens it rather than complicating it.

The trigger to document it now is [lucas42/lucos_dns#99](https://github.com/lucas42/lucos_dns/issues/99): a CI test job that runs `config-sync.py` once against real service endpoints needs a `.env` provisioned at `lucos_dns/test`, containing a system name, three service URLs, and a zones path — no secrets. lucas42 asked for the *pattern* to be designed before more test environments are created, given the access-control implications.
...
- **Pre-existing test environments predate the bright-line rule and must be retro-audited.** `lucos_contacts/test` already exists and was created before this rule, so its compliance ("no production secrets") is not guaranteed by construction — it must be confirmed by the audits ([#361](https://github.com/lucas42/lucos_creds/issues/361)/[#362](https://github.com/lucas42/lucos_creds/issues/362)). A dev-scoped key cannot read it (correctly — the access mechanism denies `test` to the agent key), so this is a human/audit check, not something self-service tooling can confirm.
