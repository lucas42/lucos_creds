...
**A test environment must not contain a production secret, under any circumstances.** This is hard policy, not a default.

The rule is chosen over the alternative (Option B: allow production secrets in test, but exclude such environments from agent keys) on **simplicity and containment** grounds — *not* on inspectability. There is in fact no way to detect a violation by inspecting the store: environment variables work by reusing the same key *names* across environments (application code reads a fixed set of names; only values differ), so a test environment's keys necessarily collide with production's by design, and the distinguishing factor — the value — is encrypted. So neither the bright-line nor Option B can be machine-audited. The bright-line still wins, on three structural points (in order of weight):

- **Legitimate test secrets are non-production *by construction* — an architectural property, not a policy hope.** Under rule 1, the normal way a secret enters a test environment is a `test → test` linked credential, which the creds server generates as a fresh random value — not a copy of anything. Under normal workflow, a bright-line violation therefore *cannot happen*; it requires a deliberate, out-of-band act. Option B has no equivalent: its safety always rests on a human classification judgment.
- **One absolute rule vs two standing invariants — fewer things to keep true over time.** This is the core security advantage. The bright-line needs neither per-environment classification nor scope management. Option B requires every test environment classified (secret-bearing or not) **and** every secret-bearing environment kept out of every agent key's scope — two invariants that drift independently and invisibly, and whose failures compound: if classification drifts you depend entirely on scope hygiene, and vice-versa. Every standing invariant is a future failure point; the bright-line has none.
- **Fewer failure paths, and the remaining one is conspicuous.** The bright-line fails only if someone manually pastes a production secret value into a test environment — an obviously-wrong act you would not stumble into. Option B reaches the same exposure two *subtle* ways, both through normal-looking administrative actions: misclassifying an environment, or adding a secret-bearing environment to an agent key's scope. So A has fewer failure paths and its one failure is more visible when it happens (a manual credential write also surfaces in loganne as an unusual operation — a soft visibility asymmetry, not enforcement).

A test that needs to authenticate to a dependency must target a **test** instance of that dependency via a `test → test` linked credential, not borrow a production secret. The rule is documented prominently wherever credentials are written (README / contributing notes); because it cannot be machine-enforced, that write-time discipline is the enforcement (see Consequences).
...
- **Test is a true peer of `development` with no extra moving parts.** Because no test environment holds a production secret (rule 2), the blast radius of `test` equals that of `development` — and that guarantee is a property of one absolute rule plus the fact that legitimate test secrets are non-production by construction, not of any classification or per-environment access carve-out that has to be kept in sync.
...
- **Authenticated integration tests against *production* services are forbidden without the gated exception.** A test that must prove it can authenticate to a live dependency has to target a *test* instance of that dependency — which may not exist yet. This pushes test-environment creation onto dependencies too (a real cost: e.g. [lucas42/lucos_dns#100](https://github.com/lucas42/lucos_dns/issues/100) may cascade into loganne/schedule_tracker needing test environments). This friction is deliberate — we choose simplicity and containment over convenience.
- **The bright-line rule cannot be machine-enforced — it rests on write-time discipline.** Because test environments reuse production's key *names* by design and values are encrypted, no audit can distinguish a legitimate test value from a copied production secret. So a violation (someone pasting a production value into a test environment) is undetectable by tooling and is prevented only by the documented policy and review at the point of writing. (Option B is no better here — it equally cannot machine-detect a misclassification — and carries the extra classification/scope-sync burden on top; this is a cost of putting any secret-bearing data behind a policy line, not a reason to prefer B.)
...
- **Allow production secrets in test environments, gated by key exclusion (Option B as default).** Rejected (with lucos-security): it requires two standing invariants — correct per-environment classification **and** correct key-scope exclusion — maintained over time, against the bright-line's single absolute rule, and it has more independent failure paths to the same exposure. Neither option is machine-auditable, so the comparison turns on simplicity and failure-surface, where the bright-line wins. Kept only as a named, signed-off exception.
