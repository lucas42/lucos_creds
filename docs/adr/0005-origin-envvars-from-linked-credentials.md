...
- **The credential store contains systems absent from `config/systems.yaml`** — `lucos_agent`, `lucos_contacts_fb_import`, `lucos_contacts_gphotos_import`, `lucos_scheduled_scripts`, `lucos_search_component`, `external_calendar`, `local_testing`, `lucos_test`, `test_app`, and two malformed entries (`set lucos_notes`, `write lucos_notes`). These are *clients*, never link targets, so they are unaffected — and none has `domain` or `http_port`, so none would gain an `INTERNAL_ORIGIN` and none could be a link target under the rule below. **Say `config/systems.yaml`, not "absent from `lucos_configy`":** four of the above (`lucos_agent`, `lucos_contacts_fb_import`, `lucos_contacts_gphotos_import` in `scripts.yaml`; `lucos_search_component` in `components.yaml`) *are* registered in configy, just not as systems — so "no configy record" is the wrong reason for the conclusion above, even though the conclusion stands. Audited across all three registries on lucas42/lucos_creds#473 (2026-08-06), which settled that a creds "system" is a *principal* while a configy "system" is a *deployed unit*, and that divergence between them is legitimate in both directions.
...
4. **Clean up malformed credential-store entries** `set lucos_notes` and `write lucos_notes` — filed and scoped as lucas42/lucos_creds#473, which also settled the "reconcile store systems absent from configy" half of this item as **needing no action**: the divergence is legitimate, and the fix is store-side identifier validation rather than a configy check. Follow-up lucas42/lucos_creds#507 covers refusing to create a namespace silently.
